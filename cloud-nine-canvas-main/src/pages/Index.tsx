...
const Index = () => {
  ...
  const renderMain = () => {
    ...
    // If not authed and we haven't triggered the gate yet, show HomeScreen in guest mode
    if (!authed && !showAuthGate) {
      return (
        <HomeScreen 
          onCloudBurst={() => {}} 
    ...
    // Show Auth Gate if not authed and we've reached the threshold (hit enter)
    if (!authed && showAuthGate) return <AuthScreen />;
