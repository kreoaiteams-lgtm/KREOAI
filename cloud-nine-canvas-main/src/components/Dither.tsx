...
float pattern(vec2 p, float t, float ws) {
  vec2 p2 = p - t * ws;
  return fbm(p + fbm(p2)); 
}
...
class RetroEffectImpl extends Effect {
  ...
  update(renderer: any, inputBuffer: any, deltaTime: any) {
    const res = (this as any).uniforms.get('resolution').value;
...
const RetroEffect = forwardRef((props: any, ref) => {
  ...
  const AnyWrappedRetro = WrappedRetro as any;
  return <AnyWrappedRetro ref={ref} colorNum={colorNum} pixelSize={pixelSize} /> as any;
});
