...
const AuthScreen = () => {
  ...
  return (
    <>
      ...
      <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-[#070b1e] py-12 px-4 md:px-6">
        {/* Shifting radial glowing lights to give vibrant and energetic backdrop */}
        <div className="absolute inset-0 z-0">
          <div className="absolute top-[-10%] right-[-10%] w-[60%] h-[60%] rounded-full bg-cyan-500/10 blur-[120px] animate-pulse" />
          <div className="absolute bottom-[-10%] left-[-10%] w-[60%] h-[60%] rounded-full bg-purple-500/10 blur-[130px] animate-pulse" />
        ...
        {/* Master Cohesive Container */}
        <motion.div 
          initial={{ opacity: 0, scale: 0.96 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          className="relative z-10 w-full max-w-5xl bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-[3rem] shadow-[0_30px_100px_rgba(0,0,0,0.5)] overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[640px]"
        >
          {/* Left Side: Elegant Unified Glass Login Terminal (5 Cols) */}
          <div className="lg:col-span-5 p-8 md:p-10 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-white/10 space-y-8 bg-black/20">
            
            {/* Branding Header */}
            <div className="space-y-4">
              <h1 
                className="text-5xl text-white tracking-tighter cursor-default font-bold uppercase select-none leading-none" 
                style={{ fontFamily: "'TAN-NIMBUS', sans-serif" }}
              >
                KREO
              </h1>
              <div className="text-[9px] font-black uppercase tracking-[0.4em] text-white/50 select-none flex flex-wrap items-center gap-x-2 gap-y-1 min-h-[1.5rem]">
            ...
            <div className="space-y-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {isSignUp && (
                    ...
                    >
                      ...
                      <input
                        ...
                        className="w-full bg-white/5 hover:bg-white/10 focus:bg-white/10 border border-white/10 focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400/20 rounded-xl pl-12 pr-4 py-3.5 text-sm text-white placeholder:text-white/30 outline-none transition-all duration-300"
                      />
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full rounded-xl bg-gradient-to-r from-blue-600 to-[#0020C2] text-white hover:from-blue-500 hover:to-[#0020C2]/95 font-semibold text-sm py-4 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 shadow-md shadow-[#0020C2]/10 disabled:opacity-50 mt-2"
                >
                  {loading ? (
              ...
              <div className="relative flex py-2 items-center">
                ...
                <span className="flex-shrink mx-4 text-white/20 text-[9px] font-black uppercase tracking-wider">or</span>
                <div className="flex-grow border-t border-white/5"></div>
              </div>

              <button
                onClick={handleGitHub}
                className="w-full rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 py-3.5 text-xs font-semibold text-white/80 transition-all flex items-center justify-center gap-2.5 active:scale-[0.98]"
              >
                <svg className="h-4 w-4 fill-current text-white" aria-hidden="true" viewBox="0 0 24 24">
            ...
            {/* Bottom Toggle */}
            <p className="text-center text-[11px] text-white/40 select-none font-medium">
              {isSignUp ? t.auth_already_have || "Already registered?" : t.auth_dont_have || "New to the Studio?"}{" "}
              <button
                onClick={() => setIsSignUp(!isSignUp)}
                className="text-yellow-400 font-bold underline underline-offset-4 hover:text-yellow-300 transition-colors"
              >
          ...
          {/* Right Side: Cozy Playful Bubbles Panel (7 Cols) */}
          <div className="lg:col-span-7 p-8 md:p-12 flex flex-col justify-between items-center text-center space-y-10 bg-white/[0.01] relative overflow-hidden">
